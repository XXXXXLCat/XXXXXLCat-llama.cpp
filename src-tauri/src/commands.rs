...
use crate::models::{match_draft, match_mmproj, scan_models, scan_models_multi};
use crate::server::ServerManager;
use crate::settings;
use crate::types::{
    AppSettings, DraftMatch, LaunchConfig, LogLine, ModelMeta, ModelProfile, MmprojMatch,
    ModelFile, ServerStatus,
};
...
/// Scan every model root and merge (de-duplicated by path). Missing roots are
/// skipped rather than failing the whole scan.
#[tauri::command]
pub fn scan_model_dirs(roots: Vec<String>) -> Result<Vec<ModelFile>, String> {
    Ok(scan_models_multi(&roots))
}

/// Upsert a named launch profile (overwrites one with the same name).
#[tauri::command]
pub fn save_profile(app: AppHandle, profile: ModelProfile) -> Result<(), String> {
...
/// Auto-resolve the speculative-decoding draft model that belongs to `model_path`.
#[tauri::command]
pub fn resolve_draft(model_path: String, root: String) -> Result<DraftMatch, String> {
    let models = scan_models(&root)?;
    Ok(match_draft(&model_path, &models))
}

/// Read GGUF header/metadata for `path` (no weight loading) so the UI can show
/// the real architecture, context length, quantisation and parameter count
/// instead of guessing from the filename.
#[tauri::command]
pub fn parse_gguf(path: String) -> Result<ModelMeta, String> {
    crate::gguf::parse_gguf(&path)
}
...
#[tauri::command]
pub async fn get_system_metrics(app: AppHandle) -> crate::metrics::SystemMetrics {
    let settings = settings::load(&app);
    let cfg = &settings.config;
    let mut metrics = crate::metrics::collect(&cfg.server_bin, &cfg.llama_dir);
    // 本软件自身版本号：取自 tauri.conf.json 的 `version` 字段（为 Option，缺失时回退到 CARGO_PKG_VERSION）。
    metrics.app_version = app
        .config()
        .version
        .clone()
